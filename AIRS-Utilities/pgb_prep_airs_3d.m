...
function [Airs,Spacing,Error,ErrorInfo,MinorErrorInfo] = pgb_prep_airs_3d(DateNum,GranuleId,options)
    arguments
        DateNum (1, 1) double
        GranuleId (1, 1) double
        options.NXT (1, 1) single {mustBeNonnegative, mustBeInteger} = 90
        options.NAT (1, 1) single {mustBeNonnegative, mustBeInteger} = 135
        options.dXT (1, 1) double {mustBeNonnegative} = 0
        options.dAT (1, 1) double {mustBeNonnegative} = 0
        options.NoDetrend logical = false
        options.KeepOldTime logical = false
        options.DayNightFlag logical = false
        options.LoadOnly logical = false
        options.NoISCheck logical = false
        options.Python logical = false
...
if options.dXT ~= 0
    options.NXT = -options.dXT;
end
options = rmfield(options,'dXT');

if options.dAT ~= 0
    options.NAT = -options.dAT;
end
options = rmfield(options,'dAT');

if exist(LocalDataDir,'file') == 7
    if isempty(options.FullDataDir) ~= 1
        options.RelDataDir = options.FullDataDir;
    elseif isempty(options.RelDataDir) ~= 1
        options.RelDataDir = fullfile(LocalDataDir,options.RelDatDir);
    else
        options.RelDataDir = [LocalDataDir,'/AIRS/3d_airs/'];
    end
else
    options.RelDataDir = options.FullDataDir;
end
options= rmfield(options,'FullDataDir');
...
if GranuleId < 1 | GranuleId > 240
    ...
    if Sign <= 0
        DayShift = -ceil(abs(DayShift));
    else      
        DayShift = floor(DayShift);
    end
